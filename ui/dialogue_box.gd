...
func _ready() -> void:
	...
	if metadata.get("type") == "delivery_contract":
		...
		# Setup custom content for racing
		label_text.visible = false
		...
		# 1. Title
		var title_lbl = Label.new()
		title_lbl.text = "Street Race"
		title_lbl.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
		title_lbl.add_theme_font_override("font", custom_font)
		title_lbl.add_theme_font_size_override("font_size", 28)
		title_lbl.add_theme_color_override("font_color", Color("#ff007f")) # Neon pink title
		title_lbl.add_theme_constant_override("outline_size", 6)
		detail_vbox.add_child(title_lbl)
		
		# 2. Row 1: Race VS Flag and Coin reward
		var row1 = HBoxContainer.new()
		row1.alignment = HBoxContainer.ALIGNMENT_CENTER
		row1.add_theme_constant_override("separation", 50)
		detail_vbox.add_child(row1)
		
		# Race Icon Control (Checkered Flag)
		var race_hbox = HBoxContainer.new()
		race_hbox.add_theme_constant_override("separation", 10)
		row1.add_child(race_hbox)
		...
		race_hbox.add_child(flag_icon)
		
		var race_lbl = Label.new()
		race_lbl.text = "VS Opponent"
		race_lbl.add_theme_font_override("font", custom_font)
		race_lbl.add_theme_font_size_override("font_size", 22)
		race_lbl.add_theme_color_override("font_color", Color.WHITE)
		race_lbl.add_theme_constant_override("outline_size", 4)
		race_hbox.add_child(race_lbl)
		...
		pin_icon.draw.connect(func():
			var center = Vector2(12, 10)
			# Pink pin body
			pin_icon.draw_circle(center, 8.0, Color("#ff007f"))
			var tri = PackedVector2Array([
				center + Vector2(-8, 2),
				center + Vector2(8, 2),
				center + Vector2(0, 18)
			])
			pin_icon.draw_colored_polygon(tri, Color("#ff007f"))
			pin_icon.draw_circle(center, 3.0, Color.WHITE)
		)
		row2.add_child(pin_icon)
